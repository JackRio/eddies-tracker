...
// Shows when this deploy of the site went live (docs/build-info.json,
// bumped on every push) and, on pages that pass one, when the data itself
// was last regenerated by the desktop app's Publish Site button - so a
// stale phone/tab (or the GitHub Pages CDN taking a minute to catch up)
// is obvious at a glance instead of silently showing old code or data.
async function renderBuildInfo(dataGeneratedAt) {
  const el = document.getElementById('build-info');
  if (!el) return;
  try {
    const { deployedAt } = await fetchJson('build-info.json');
